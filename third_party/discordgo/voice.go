...
type VoiceConnection struct {
	...
	// Coalesce Discord's common double VOICE_SERVER_UPDATE so we don't
	// cancel a half-open websocket/UDP handshake and orphan the DAVE session
	// WaitForDAVEReady is watching.
	voiceServerUpdateGen uint64
	pendingVoiceEndpoint string
	pendingVoiceToken    string
...
func (v *VoiceConnection) WaitForDAVEReady(ctx context.Context) error {
	...
	// Always re-read v.dave: a mid-connect VOICE_SERVER_UPDATE recreates the
	// DAVE session on op4. Snapshotting the pointer once leaves this wait
	// stuck on a dead session while Welcome activates the replacement.
	for {
		if v.Status == VoiceConnectionStatusDead {
			if v.Err != nil {
				return fmt.Errorf("voice connection dead while waiting for DAVE: %w", v.Err)
			}
			return errors.New("voice connection dead while waiting for DAVE")
		}
		dave := v.dave
		if dave == nil {
			return nil
...
func (v *VoiceConnection) onVoiceServerUpdate(ev *VoiceServerUpdate) (err error) {
	...
	// If no endpoint, cancel any in-flight connect and wait for the next event.
	if ev.Endpoint == nil {
	...
	// Already ready on this endpoint — ignore duplicate updates that would
	// tear down a live DAVE session for no gain.
	if v.Status == VoiceConnectionStatusReady && v.voiceEndpoint == endpoint && v.wsCancel != nil {
		v.log(LogInformational, "ignoring duplicate VOICE_SERVER_UPDATE for ready endpoint")
		return
	}
...
// ErrVoiceSessionInvalid means Discord closed the voice websocket with 4006
	...
	for i := 0; i < 100; i++ {
		...
		for {
			...
			if err != nil {
				...
				// 4006: session/token invalidated. Soft reconnect cannot recover;
				// callers must leave the channel and fully rejoin.
				if websocket.IsCloseError(err, 4006) {
					v.failure(fmt.Errorf("%w: %w", ErrVoiceSessionInvalid, err))
					return
				}
	...
	if binary {
		...
		case 4: // udp encryption secret key
			...
			if op4.DAVEProtocolVersion > 0 {
				dave, daveErr := NewDAVESession(v.session.State.User.ID, v.ChannelID, op4.DAVEProtocolVersion)
				if daveErr != nil {
					v.Cond.L.Unlock()
					v.failure(fmt.Errorf("DAVE session initialization failed: %w", daveErr))
					return
				}
				v.dave = dave
				...
				// Wake WaitForDAVEReady so it picks up the new session pointer.
				v.Cond.Broadcast()
			}
...
func (v *VoiceConnection) handleDAVEBinary(message []byte) {
	...
	opcode := message[2]
	v.log(LogInformational, "DAVE binary opcode=%d len=%d", opcode, len(message)-3)

	v.Cond.L.Lock()
	dave := v.dave
	v.Cond.L.Unlock()
	if dave == nil {
		v.log(LogWarning, "DAVE binary opcode %d received without a session", opcode)
		return
	}

	result, err := dave.HandleGatewayBinary(message)
	if err != nil {
		v.log(LogError, "DAVE binary opcode %d failed: %s", opcode, err)
		return
	}
	...
	if result.SendTransitionReady && result.TransitionID != nil {
		v.log(LogInformational, "DAVE transition ready id=%d canEncrypt=%v", *result.TransitionID, dave.CanEncrypt())
		v.sendDAVEReadyForTransition(*result.TransitionID)
		v.Cond.Broadcast()
	}
}
...
func (v *VoiceConnection) sendDAVEBinary(message []byte) {
	v.Cond.L.Lock()
	wsConn := v.wsConn
	v.Cond.L.Unlock()
	if wsConn == nil {
	...
	v.wsMu.Unlock()
	if err != nil {
		v.log(LogError, "DAVE binary send failed: %s", err)
	}
}
