...
- **send_message(text, buttons?, selects?)** — Send a collaborator message (Markdown ok). Optional `buttons` are rows of `{text, data, style?, url?}` for decisions/approvals — not every chatty reply. Optional `selects` are Discord dropdowns (`{id, placeholder?, options}`). When the collaborator taps a button or picks an option, you receive it as a collaborator message.
- **send_rich_message(content, title?, color?, fields?, buttons?, selects?)** — Send a structured digest (daily summary, status card). Prefer for formatted digests; not for ordinary chat. Discord renders an embed; Telegram uses rich markdown (buttons when provided).
