...
- If the same kind of action keeps producing no progress, that's a signal to edit the prompt that's pushing you toward it. Prefer `prompts/agent.md` for personal habits; use `prompts/continue.md` when the continue turn itself is wrong. Only touch `prompts/system.md` for shared rulebook fixes (or when a migration / collaborator asks). Log the edit in `prompts/changelog.md`. That is part of the work, not a distraction from it.
