...
func (d *DAVESession) CanEncrypt() bool {
	d.mu.Lock()
	defer d.mu.Unlock()
	// frameCipher is set after Welcome + DeriveSenderKey. The `active` flag
	// can lag (or stay false when OP22 never arrives for the initial
	// transition); opusSender only needs a cipher.
