...
Embedded HTTP admin UI driving adapter. Stdlib `net/http` only — no chi/gin/echo — to match the project's minimal-deps posture. Bound to a loopback address by default; reverse-proxy TLS termination is the documented path for remote access. There is no built-in TLS.

The UI is a multi-section dashboard shell — persistent sidebar (Dashboard / Configuration / Subagents / Skills / Version / Logs) plus a topbar — themed as a Matrix terminal (phosphor-on-black, scanline overlay, vendored VT323 + Share Tech Mono webfonts under `assets/fonts/`, custom `[data-theme="matrix"]` block in `assets/terminal.css`). Each section is its own page; full-document navigation is replaced by HTMX `hx-boost` on the body so the head (fonts, CSS) never reloads — without this the WOFF2 fonts visibly re-applied mid-paint on every link click.
...
- **`handlers_configuration.go` + `config_form.go`**: the Configuration page is rendered automatically from `*config.Config` via reflection — `BuildConfigForm(cfg)` walks the struct emitting `ConfigSection` / `ConfigField` records keyed by their dotted toml path, classifying each leaf into a `ConfigKind` (bool, int, float, string, secret, duration, select, readonly). Adding a new field anywhere in `internal/config` wires it into the form automatically. Secrets are detected by Go field-name suffix (`Key` / `Password` / `Token`) and rendered as masked inputs with leave-blank-to-keep semantics; selects opt in via the small `selectOptions` lookup; `time.Duration` is detected via the unexported `config.duration` alias by package + name. `[]SubagentProfile` (and any future `[]struct`) renders as a repeating sub-form with index-in-path keys (`subagent.profiles.0.name`). The save handler reads the on-disk TOML as a baseline (so secrets the form didn't repost are preserved), runs `ApplyConfigForm` to overlay form values, encodes the modified struct back to TOML via `BurntSushi/toml`, and writes through the existing `ConfigStore.Write` (which validates atomically). `configuration.html` renders the sections as a wrap-around tab strip — all section tabs visible at once via `flex-wrap`, custom `.matrix-tab` styling, with a tiny inline JS handler that swaps the active panel and persists the choice across navigations in `sessionStorage`.
- **`handlers_config.go`**: only the small `/admin/config/restart` handler — POSTed by the Configuration page's "restart agent now" button after a successful save to fire `ConfigStore.Restart` (which closes the same shutdown channel SIGINT does).
- **`handlers_skills.go`** / **`handlers_update.go`**: per-skill enable/disable toggle (`POST /admin/skills/toggle`) and self-update apply (`POST /admin/update/apply`). Both re-render their fragment template after the action so HTMX can hx-swap the new state without a page reload.
- **`inspector.go`**: read-side ports the dashboard consumes — `AgentInspector` (single method `Snapshot() agent.AgentSnapshot`), `SubagentInspector` (`Status` + `Profiles`), `SkillsAdmin`, `UpdateInspector`, `ConfigStore`, all defined consumer-side in adminhttp. `ToolBuffer` is the in-memory ring buffer that satisfies `tools.Observer`; capacity defaults to 500. The same `*ToolBuffer` is handed to the primary `tools.Executor` as its `Observer`.
