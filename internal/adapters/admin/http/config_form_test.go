...
func TestBuildConfigForm_CoversEverySection(t *testing.T) {
	...
	wantSections := []string{
		...
		"subagent", "admin", "peers", "publish", "webhook",
	}
...
func TestBuildConfigForm_ClassifiesFieldsByType(t *testing.T) {
	...
	cases := []spec{
		...
		{"publish.bind", KindString},
		{"publish.root", KindString},
		{"publish.public_base_url", KindString},
		{"publish.md_template", KindString},
